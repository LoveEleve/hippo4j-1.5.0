...
public class ThreadPoolConfig {
    ...
    public ThreadPoolExecutor messageProducePool() {
        return ThreadPoolBuilder.builder().threadFactory(producePoolName).dynamicPool().threadPoolId("test-bug").build();
    }

    @SpringDynamicThreadPool
    public ThreadPoolExecutor messageBugPool() {
        return ThreadPoolBuilder.builder().threadFactory("test-bug-2").dynamicPool().threadPoolId("test-bug-2").build();
    }

    @SpringDynamicThreadPool
    public ThreadPoolExecutor BugPool() {
        return ThreadPoolBuilder.builder().threadFactory("test-bug-3").dynamicPool().threadPoolId("test-bug-3").build();
    }

    @SpringDynamicThreadPool
    public ThreadPoolExecutor BugFixPool() {
        return ThreadPoolBuilder.builder().threadFactory("bug-fix").dynamicPool().threadPoolId("bug-fix").build();
    }
